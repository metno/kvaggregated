...
namespace {
void myDebug(const std::string &message,
             const kvalobs::serialize::KvalobsData &d) {
...
void setupPidFile(dnmi::file::PidFileHelper &pidFile) {
  ...
  std::string pidFileName =
      dnmi::file::createPidFileName(kvPath("rundir"), "kvAgregated");

  bool pidfileError;
  if (dnmi::file::isRunningPidFile(pidFileName, pidfileError)) {
    if (pidfileError) {
      std::ostringstream msg;
      msg << "An error occured while reading the pidfile " << pidFileName
          << ". Remove the file if it exist and "
             "<agregate> is not running. If it is running and there are "
             "problems. Kill <agregate> and restart it.";
      throw std::runtime_error(msg.str());
    } else {
      std::ostringstream msg;
      msg << "Is <agregate> allready running? If not remove the pidfile: "
          << pidFileName;
      throw std::runtime_error(msg.str());
      ;
    }
...
void createLog(const std::string &logFileName, milog::LogLevel level,
               int maxSize) {
  ...
  else if (not is_directory(logDir))
...
void runThreadWithBackProduction(BackProduction &back,
...
void runAgregator(const AggregatorConfiguration &conf,
                  kvservice::DataAccess &dataAccess,
                  kvservice::proxy::CallbackCollection &callbacks) {
  if (conf.backProduction()) {
    BackProduction back(callbacks, nullptr, conf.backProductionSpec(),
                        conf.stations());
  ...
  boost::posix_time::ptime to(
      now.date(), boost::posix_time::hours(now.time_of_day().hours()));
...
int main(int argc, char **argv) {
  ...
  try {
    ...
    if (!conf.backProduction()) {
      ...
      setMetricsLogfile("kvAgregated_metrics.log",
                        kvalobs::kvPath(kvalobs::logdir));
    }

    // KvApp
    LOGINFO("Programname used in configuration lookup: '"
            << miutil::getProgramName() << "'");
    std::unique_ptr<kvservice::KvApp> app(
        kvservice::KvApp::create(miutil::getProgramName(), argc, argv));

    // Proxy database
    kvservice::proxy::CallbackCollection callbacks;

    // std::unique_ptr<kvservice::DataAccess> dataAccess(new
    // kvservice::KvalobsDataAccess);
    ...
    if (conf.proxyDatabaseName() != "") {
      ...
      dataAccess.reset(new kvservice::proxy::KvalobsProxy(
          conf.proxyDatabaseName(), conf.repopulateDatabase()));
    } else {
