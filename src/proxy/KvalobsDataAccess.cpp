...
struct invalid {
  ...
  bool operator()(const kvalobs::kvData &data) {
    return not(
        paramid == data.paramID() and type == data.typeID() and
...
void KvalobsDataAccess::getData(KvDataList &data, int station,
                                const boost::posix_time::ptime &from,
                                const boost::posix_time::ptime &to, int paramid,
...
void KvalobsDataAccess::getAllData(KvDataList &data,
                                   const boost::posix_time::ptime &from,
...
KvalobsDataAccess::sendData(const KvDataList &data) {
  if (!KvApp::kvApp)
  ...
  std::string msg =
      kvalobs::serialize::KvalobsDataSerializer::serialize(toSend);
...
{
bool compatible(const kvalobs::kvStationMetadata &metadata,
                const kvalobs::kvData &d) {
  if (metadata.stationID() != d.stationID())
    return false;
  if (not metadata.fromtime().is_not_a_date_time() and
      metadata.fromtime() > d.obstime())
    return false;
  if (not metadata.totime().is_not_a_date_time() and
      metadata.totime() < d.obstime())
    return false;
...
const kvalobs::kvStationMetadata *
mostSpecific(const kvalobs::kvStationMetadata *a,
...
float KvalobsDataAccess::getStationMetadata(
    const std::string &metadataName, const kvalobs::kvData &validFor) const {
  if (!KvApp::kvApp)
  ...
  try {
    if (!KvApp::kvApp->getKvStationMetaData(metadata, validFor.stationID(),
                                            validFor.obstime(), metadataName)) {
      metrics->kvDb.stop(true);
    ...
    msg << "Unable to find metadata for stationid " << validFor.stationID()
        << " and metadataName " << metadataName << " for obstime " << validFor
        << ". Error: " << e.what();
    throw std::runtime_error(msg.str());
  }

  const kvalobs::kvStationMetadata *ret = 0;
  for (std::list<kvalobs::kvStationMetadata>::const_iterator it =
           metadata.begin();
       it != metadata.end(); ++it)
  ...
  if (!ret) {
    std::ostringstream msg;
    msg << "Unable to find metadata: " << metadataName << " for data "
        << validFor;
    metrics->kvDb.stop(true);
    throw std::runtime_error(msg.str());
  }
