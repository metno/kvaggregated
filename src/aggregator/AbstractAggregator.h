...
public:
  ...
  /**
   * \brief determine if we are interested in the given piece of data, based on
   * its obstime
   */
   ...
   * \brief A time range.
   */
  typedef std::pair<boost::posix_time::ptime, boost::posix_time::ptime>
      TimeSpan;
   ...
   */
  const ParameterList &readParam() const { return read_param; }
...
protected:
   ...
   *
   * \param corrected The corrected agregated value for the observation. This
   * may have the value \c invalidParam.
   *
   ...
   */
  kvDataPtr getDataObject(const kvalobs::kvData &trigger,
                          const boost::posix_time::ptime &obsTime,
                          double original, double corrected,
                          const std::list<kvalobs::kvData> &sourceData);
...
private:
  /**
   * Calculate value for new controlinfo(0) - fagg, based on the source data
   * which aggregation was made on.
   */
